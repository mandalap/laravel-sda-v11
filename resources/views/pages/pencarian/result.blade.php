...
<div id="Content-Container" class="relative flex flex-col w-full max-w-[640px] min-h-screen mx-auto bg-white overflow-x-hidden">
    ...
    <section id="Result" class="flex relative flex-col gap-4 px-5 mt-5 mb-9">
        ...
        <a href="{{ route('detailproject', [$item->project->jenis->slug, $item->project->kategori->slug, $item->project->slug]) }}" class="card">
            <div class="flex rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white hover:border-[#d40065] transition-all duration-300">
                <div class="flex w-[120px] h-[183px] shrink-0 rounded-[30px] bg-[#D9D9D9] overflow-hidden">
                    <div class="relative">
                        ...
                        <img src="{{ asset('storage/' . $item->project->thumbnail) }}" class="object-cover w-full h-full" alt="icon">
                    </div>
                </div>
                <div class="flex flex-col gap-3 w-full">
                    <h3 class="font-semibold text-lg leading-[27px] line-clamp-2 min-h-[54px]">{{ $item->project->nama_project }}</h3>
                    <hr class="border-[#F1F2F6]">
                    <div class="flex items-center gap-[6px]">
                        <img src="assets/images/icons/location.svg" class="flex w-5 h-5 shrink-0" alt="icon">
                        <p class="text-sm text-ngekos-grey">{{ $item->project->lokasi->regency->name }}</p>
                    </div>
                    <div class="flex items-center gap-[6px]">
                        <img src="assets/images/icons/profile-2user.svg" class="flex w-5 h-5 shrink-0" alt="icon">
                        <p class="text-sm text-ngekos-grey">{{ $item->project->kategori->kategori }}</p>
                    </div>
                    <hr class="border-[#F1F2F6]">
                    @php
                        $harga = $item->project->project_product->min('harga');
                        $diskon = $item->project->project_product->min('discount'); // Asumsi diskon dalam persen
